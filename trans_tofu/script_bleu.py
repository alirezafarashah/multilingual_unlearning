...
def main():
    ...
    parser.add_argument(
        "--bleu_csv_path",
        type=str,
        default="bleu_scores.csv",
        help="Path to save BLEU scores CSV file.",
    )
    ...
    # 5) Compute BLEU (Per Column AND Global)
    print("\n========== BLEU SCORES (original EN vs back-translated EN) ==========")

    # -------------------------------------------------------------------
    bleu_results = []  # store for CSV
    
    # Accumulators for the global score
    all_original_texts = []
    all_back_translated_texts = []

    for col in text_columns:
        # Ensure data is string format
        orig_col = [str(x) for x in original_dataset[col]]
        back_col = [str(x) for x in back_translated_data[col]]

        # 1. Calculate Per-Column BLEU
        # We wrap orig_col in a list because sacrebleu expects a list of reference lists
        bleu_score = corpus_bleu(back_col, [orig_col]).score
        ...
        # 2. Accumulate for Global BLEU
        all_original_texts.extend(orig_col)
        all_back_translated_texts.extend(back_col)

    # -------------------------------------------------------------------
    # Calculate Global BLEU
    # -------------------------------------------------------------------
    # We pass the combined lists to sacrebleu
    global_bleu = corpus_bleu(all_back_translated_texts, [all_original_texts]).score
    
    print(f"---------------------------------")
    print(f"GLOBAL DATASET BLEU: {global_bleu:.2f}")
    print(f"---------------------------------")

    # Add global score to the results list for the CSV
    bleu_results.append({
        "column": "GLOBAL_AVERAGE", # Naming it Global
        "bleu": global_bleu
    })

    # 6) Save BLEU scores to CSV
    csv_path = args.bleu_csv_path
    with open(csv_path, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["column", "bleu"])
        writer.writeheader()
        writer.writerows(bleu_results)

    print(f"\nBLEU scores (including global) saved to {csv_path}")
    print("Done.")
